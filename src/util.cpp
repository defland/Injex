...
const char * const BITCOIN_CONF_FILENAME = "injex.conf";
const char * const BITCOIN_PID_FILENAME = "injexd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "injex" is a composite category enabling all Injex-related debug output
            if(ptrCategory->count(string("injex"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\InjexCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\InjexCore
    // Mac: ~/Library/Application Support/InjexCore
    // Unix: ~/.injexcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "InjexCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/InjexCore";
#else
    // Unix
    return pathRet / ".injexcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty injex.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override injex.conf
