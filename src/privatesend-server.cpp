...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Injex related functionality
    ...
    if(strCommand == NetMsgType::DSACCEPT) {
        ...
        if(vecSessionCollaterals.size() == 0 && mnInfo.nLastDsq != 0 &&
            mnInfo.nLastDsq + mnodeman.CountEnabled(CPrivateSend::ActiveProtocol())/5 > mnodeman.nDsqCount)
        {
        ...
        if(!dsq.fReady) {
            ...
            int nThreshold = mnInfo.nLastDsq + mnodeman.CountEnabled(CPrivateSend::ActiveProtocol())/5;
...
/*
    ...
    allow endless transaction that would bloat Injex and make it unusable. To
